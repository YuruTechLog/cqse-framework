...
**目的:** 比喩・用語の種まき章。読者を景色に引き込み、以降の章の前提を揃える。説明より情景描写優先。

- **地形の成り立ち**
  AIの内部 = 学習データが分解・積み重なった広大な地形。
  よく使われる意味の組み合わせ = 低地・谷・川・集落。
  滅多に使われない組み合わせ = 高地（例: ヒマラヤ山頂）。川も集落もない。

- **力学の描写**
  プロンプト（AIへの入力文）を投げかける → 磁石のように地形上のかけらを引き寄せる → 重力で川へ流れ落ちる → 混ざり合って出力になる。
  何もしなければ「一番近い集落」に流れ着く。意図した集落かどうかは関係ない。

- **測量者の認識**
  地形は変えられない。でも傾けられる・歪められる。
  その認識を持った瞬間 = 測量者のポジション。

- **研究者との接続**
  Activation Steering研究 = 研究者がラボでやっていること。
  Approach Axisはその地図をプロンプト設計者の手に渡す試み。

- **用語の種まき一覧**
  意味の地形 / 谷底の川・集落 / ヒマラヤ山頂 / プロンプト（AIへの入力文）/ 磁石・砂鉄（第2章で展開）/ 合金（第5章で展開）/ 地形折り畳み（第8章で展開）

> ドラフト: `drafts/prologue_draft_v3.md`
